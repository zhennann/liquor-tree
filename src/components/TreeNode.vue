<template>
  <li class="tree-node" :class="nodeClass">
    <div class="tree-content" :style="{'padding-left': paddingLeft}" @click.stop="select">
      <i class="tree-arrow" :class="{'expanded': node.states.expanded, 'has-child': node.children.length || node.isBatch}" @click.stop="toggleExpand">
      </i>
      ...
      <a href="javascript:void(0)" class="tree-anchor" tabindex="1" ref="anchor" @focus="onNodeFocus" @dblclick="tree.$emit('node:dblclick', node)">
        <node-content :node="node" />
      </a>
    </div>
    <transition name="l-fade">
      <ul v-if="hasChildren() && state.expanded" class="tree-children">
        <node v-for="child in node.children" v-if="child.visible()" :key="child.id" :node="child" :options="options">
        </node>
      </ul>
    </transition>
  </li>
